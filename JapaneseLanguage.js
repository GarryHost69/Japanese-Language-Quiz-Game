...
function generateImage() {
	if (choice == 0) {
		img = Object.values(images);
		let temp = floor(random(0, img.length));
		while (temp == ImageIndex) {
			temp = floor(random(0, img.length));
		}
		ImageIndex = temp;
		loaded = true;
		if (ImageIndex > 0 && ImageIndex < img.length - 1) {
			wrongIndex1 = ImageIndex + 1;
			wrongIndex2 = ImageIndex - 1;
		}
		else
		if (ImageIndex == 0) {
			wrongIndex1 = ImageIndex + 1;
			wrongIndex2 = ImageIndex + 2;
		}
		else
		...
		wrong1 = Object.values(img[wrongIndex1]);
		wrong1 = wrong1[1].HiraganaWord;
		wrong2 = Object.values(img[wrongIndex2]);
		wrong2 = wrong2[1].HiraganaWord;
		sound = Object.values(img[ImageIndex]);
		sound = sound[1].HiraganaWord;
		img = Object.values(img[ImageIndex]);
		img = img[0].fname;
		img = loadImage(img);
	...
	image(img, 450, 100, 400, 300);
}

function HiraganaVocabGame() {
	generateImage();
	strokeWeight(2);
	fill(200, 250, 220);
	rect(20, 20, 100, 50);
	...
	textSize(30);
	text("Select the Correct Hiragana word for the image being displayed", 335, 25, 1135, 50);
	switch (correctChoice) {
		case 1:	text(sound, 200, 510, 400, 550);
				text(wrong1, 600, 510, 800, 550);
				text(wrong2, 1000, 510, 1300, 550);
				break;
		case 2:	text(sound, 600, 510, 800, 550);
				text(wrong2, 200, 510, 400, 550);
				text(wrong1, 1000, 510, 1300, 550);
				break;
		case 3:	text(sound, 1000, 510, 1300, 550);
				text(wrong1, 200, 510, 400, 550);
				text(wrong2, 600, 510, 800, 550);
				break;
	}
}
...
function displayMenu() {
	...
	text("Hiragana Vocabulary Quiz", 520, 410, 1200, 200);
}
...
function mouseClicked() {
	switch (frame) {
		...
		case 2:	if (mouseX >= 20 && mouseX <= 120) {
				...
				op3 = false;
				break;
		case 3:	if (mouseX >= 20 && mouseX <= 120) {
					if (mouseY >= 20 && mouseY <= 70) {
						frame = 0;
						loaded = false;
						score = 0;
